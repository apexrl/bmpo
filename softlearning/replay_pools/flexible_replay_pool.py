...
class FlexibleReplayPool(ReplayPool):
    ...
    def last_n_random_batch(self,last_n,batch_size,field_name_filter=None, **kwargs):
        last_n_random_indices = np.random.randint(
            self._pointer - min(self.size, last_n), self._pointer,batch_size
        ) % self._max_size
        return self.batch_by_indices(
            last_n_random_indices, field_name_filter=field_name_filter, **kwargs)
